...
const getDetail = (req, res, id) => {
  fs.readFile("./people.txt", "utf-8", (err, data) => {
    ...
    //   }
    // });
    const isFind = JSON.parse(data).find((row) => row.id === id);
...
const createData = (payload) => {
  fs.readFile("./people.txt", "utf8", (err, data) => {
    ...
    fs.writeFile("./people.txt", JSON.stringify(parsedData), "utf8", (err) => {
      if (err) throw err;
...
const deleteData = (req, res, id) => {
  fs.readFile("./people.txt", "utf8", (err, data) => {
    ...
    // fs.writeFile("./people.txt", JSON.stringify(index), "utf8", (err) => {
    //   if (err) throw err;
    //   console.log(index);
    // });
